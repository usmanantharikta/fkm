    ...
    */
    ...
    Views.BlockControl = Backbone.Marionette.View.extend({
        ...
        events: {
            ...
            // Custom filter
            // @todo ??i t�m ra c�ch extend event tr�n backbone version m?i
    ...
    Views.Carousel = Backbone.View.extend({
        ...
        setupView: function() {
                ...
                success: function(res) {
                    if (res.success) {
                        ...
                        if (res.data.length >= that.maxFileUpload) {
                            if( $('#'+that.options.uploaderID+'_browse_button').length > 0 ){
                                $('#'+that.options.uploaderID+'_browse_button').parent().hide('slow');
                            }
                            else {
            ...
            if (typeof this.carousel_uploader === 'undefined') this.carousel_uploader = new Views.File_Uploader({
                ...
                success: function() {
                    var max_files = that.maxFileUpload;
                    if($('.' + that.options.uploaderID+'-image-list').find('li.image-item').size() > 0 ){
                        j = $('.' + that.options.uploaderID+'-image-list').find('li.image-item').size();
                    }
        ...
        removeCarousel: function(event) {
            ...
            if( this.numberOfFile > this.minFileUpload ) {
                ...
                $.ajax({
                    ...
                    success: function () {
                        if( $('#'+that.options.uploaderID+'_browse_button').length > 0 ){
                            $('#'+that.options.uploaderID+'_browse_button').parent().show('slow');
                        }
                        else {
